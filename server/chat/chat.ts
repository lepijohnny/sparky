...
export function createChatWorkspace(
  ...
  bus.on("chat.turn.delete", (data) => crud.deleteTurn(data));
  bus.on("chat.entry.edit", (data) => crud.editEntry(data));
  ...
  bus.on("chat.summarize", async (data) => {
    const ok = await conversation.forceSummarize(data.chatId);
    return { ok };
  });

...
export const TrustScope = z.enum(["read", "write", "bash", "bus"]).describe("Trust scope: read/write gate file paths, bash gates shell commands, bus gates app events");
export const TrustList = z.enum(["allow", "deny", "ask"]).describe("Rule list: allow auto-approves, deny blocks, ask prompts the user");
export const TrustMode = z.enum(["read", "write", "execute"]).describe("Permission mode: read = browse only, write = read + file edits, execute = full access including shell");
...
export const RuleRemoveSchema = z.object({
  scope: TrustScope,
  list: TrustList,
...
registerRoleGrammar("permissions", defineGrammar("Rule model", [
